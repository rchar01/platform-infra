  ...
  validation {
    condition = length(var.vms) == 0 || (
      ...
      alltrue([
        for _, vm in var.vms :
        vm.cores == 2 &&
        coalesce(vm.cpu_type, "") == "host" &&
        vm.memory_mb == 2048 &&
        vm.memory_floating_mb == null &&
        vm.disk_gb == 20 &&
        coalesce(vm.boot_disk_interface, "scsi0") == "scsi0" &&
        length(vm.additional_disks) == 0
      ])
    )
    error_message = "migration-test must define exactly two 2-core, fixed 2048 MiB, host-CPU VMs with one 20 GiB scsi0 boot disk and no additional disks."
  }

  validation {
    condition = length(var.vms) == 0 || (
      toset([for _, vm in var.vms : vm.expected_version_id]) == toset(["10.0", "10.1"]) &&
      alltrue([for _, vm in var.vms : vm.template_vm_id != null]) &&
      length(toset([for _, vm in var.vms : vm.template_vm_id])) == 2
    )
    error_message = "migration-test must define explicit, distinct templates for exactly one Rocky 10.0 and one Rocky 10.1 baseline."
  }
}
